...
namespace Kamgam.SettingsGenerator
{
    ...
    public static class SettingsSerializer
    {
        public static string ToJson(Settings settings)
        {
            ...
            // Compression
            // There is a lot of unneeded data in there. We can strip it but
            // that takes time so we only do it if the text is rather long.
            // Be aware that this may fail to reduce the size if the JsonUtility
            // serialization changes in the future.
            int charLimit = 1;//10000;
            if (json.Length > charLimit)
            {
                json = json.Replace(",\"IntValues\":[]", "");
                json = json.Replace(",\"FloatValues\":[]", "");
                json = json.Replace(",\"BoolValues\":[]", "");
                json = json.Replace(",\"StringValues\":[]", "");
            }
